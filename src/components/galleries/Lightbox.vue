<template>
  ...
  >
    <div ref="backdrop" class="lightbox-backdrop" :style="backdropStyle"></div>
...
export default {
  ...
  computed: {
    ...
    backdropStyle() {
      ...
      if (this.$data.$_swipable_data.type === types.VERTICAL) {
        const distance = Math.abs(this.$data.$_swipable_data.dragOffsetY);

        opacity = curve(
          Math.max(
            0,
            (window.innerHeight / 2 - distance) / (window.innerHeight / 2)
          )
        );
      }
  ...
  methods: {
    ...
    async close() {
      if (this.state !== states.DRAGGING && this.state !== states.IDLE) return;
